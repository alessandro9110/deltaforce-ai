...
df_install_skills() {
    local skills output dest="$DF_TARGET_DIR/.claude/skills"
    ...
    output=$("$DF_DATABRICKS" aitools install --path "$(df_native_path "$dest")" --skills "$skills" 2>&1) \
        || { printf '%s\n' "$output" >&2; df_die "databricks aitools install failed"; }
