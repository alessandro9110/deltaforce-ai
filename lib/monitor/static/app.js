...
function featureCard(feature) {
  ...
  if (feature.column === "todo") {
    ...
    middle = `<span class="chip po">Your review</span>`;
    foot = progressHtml(feature);
  } else {
...
function featureDrawer(feature) {
  ...
  const review = feature.status === "awaiting_po"
    ? `<div class="callout po"><strong>Waiting for your review.</strong> In Claude Code: <code>/df-approve ${esc(feature.id)}</code> or ` +
      `<code>/df-changes ${esc(feature.id)} …</code>${feature.review_report ? ` · <a href="#doc/${esc(feature.review_report)}">Read the review report</a>` : ""}</div>`
    : "";
