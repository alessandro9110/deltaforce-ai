...
function auditDrawer(s) {
  const audit = s.audit || { available: false, roles: [], denials: [], production: [], counts: { denied: 0, production_reads: 0 } };
  const head = drawerHead("Audit", "What was blocked, and what was read on production");
  if (!audit.available) {
    return `${head}<p class="muted">Nothing audited yet. Every Databricks call, shell command and guardrail decision of every agent is recorded in <span class="mono">.deltaforce/audit.jsonl</span>.</p>`;
  }

  const tiles =
    `<div class="tiles">` +
    `<div class="tile ${audit.counts.denied ? "warn" : ""}"><strong>${audit.counts.denied}</strong><span class="label">Blocked</span>` +
    `<span class="muted small">${audit.counts.denied ? "actions the guardrails refused" : "nothing was refused"}</span></div>` +
    `<div class="tile"><strong>${audit.counts.production_reads}</strong><span class="label">Production reads</span>` +
    `<span class="muted small">${audit.counts.production_reads ? "read-only calls on the production workspace" : "production was never read"}</span></div>` +
    `<div class="tile"><strong>${compact(audit.calls)}</strong><span class="label">Audited calls</span>` +
    `<span class="muted small">${audit.partial ? "recent part of the file" : "since the project started"}</span></div></div>`;

  const roles = audit.roles.length
    ? `<div class="table-wrap"><table class="grid"><thead><tr><th>Role</th><th class="num">Blocked</th><th class="num">Production reads</th></tr></thead><tbody>` +
      audit.roles.map((role) =>
        `<tr><td>${roleBadge(role.role)} ${esc(role.title)}</td><td class="num">${role.denied || "—"}</td>` +
        `<td class="num">${role.production_reads || "—"}</td></tr>`).join("") +
      `</tbody></table></div>`
    : `<p class="muted">No role was blocked and nobody read production.</p>`;

  const rows = (items, withReason) => items.length
    ? `<ol class="audit-list">${items.map((item) =>
        `<li><time>${esc(when(item.ts))}</time>${roleBadge(item.role)}` +
        `<span class="audit-what"><span class="mono">${esc(item.tool)}</span>` +
        `${withReason ? `<span class="warn-text">${esc(item.reason || "blocked")}</span>` : ""}` +
        `${item.input ? `<code>${esc(item.input)}</code>` : ""}</span></li>`).join("")}</ol>`
    : null;

  const denials = rows(audit.denials, true) || `<p class="muted">The guardrails never had to block anything.</p>`;
  const production = rows(audit.production, false)
    || `<p class="muted">Nobody read the production workspace${s.project.workspace ? "" : " (none is configured)"}.</p>`;

  return (
    head +
    `<p class="muted small">Every call of every agent is audited with its role. This page reads the recent part of the file; nothing is ever sent anywhere.</p>` +
    tiles +
    `<h3 class="drawer-section">Per role</h3>${roles}` +
    `<h3 class="drawer-section">Blocked <span class="muted">newest first</span></h3>${denials}` +
    `<h3 class="drawer-section">Production reads <span class="muted">read-only, audited</span></h3>${production}`
  );
}
