...
const app = {
  ...
  featuresView: stored.get("deltaforce.featuresView", "board"),
...
function featureCard(feature) {
  ...
  return (
    ...
    `<span class="fid">${esc(feature.id)}</span><h4>${esc(feature.title)}</h4>${description}${middle}${foot}</a>`
  );
}
...
function backlogItem(feature) {
  ...
  return (
    ...
    `<a class="backlog-title" href="#feature/${esc(feature.id)}">${esc(feature.title)}</a>${statusChip(feature)}</div>` +
...
// Keep expanded task lists open when the page refreshes its data.
