...
def write_claude_settings(config: Mapping[str, Any], paths: ProjectPaths) -> Path:
    ...
    # Guardrail and audit hooks: committed, and written through ${CLAUDE_PROJECT_DIR} so they hold on any machine
    # that installed the project. Anyone who clones without installing has no runtime, and a hook that cannot start
    # does not block - the installer stays the way in.
...
def write_claude_settings_local(config: Mapping[str, Any], paths: ProjectPaths) -> Path:
    ...
    # The hooks moved to the committed settings.json: clear the registration older installs left here.
    local_hooks = guardrails.without_deltaforce_hooks(data.get("hooks"))
    if local_hooks:
        data["hooks"] = local_hooks
    else:
        data.pop("hooks", None)
...
def write_df_wrapper(_: Mapping[str, Any], paths: ProjectPaths) -> Path:
    """The project CLI. Every path is resolved from the script itself, so no path of this machine ends up in it."""
    versions = load_versions()

    def under_root(path: Path) -> str:
        """`"$root"/<relative>` for anything inside the project; an absolute path for a framework kept outside it."""
        try:
            return '"$root"/' + shlex.quote(path.resolve().relative_to(paths.root.resolve()).as_posix())
        except ValueError:
...
# Generated by the installer; not committed.
set -euo pipefail
root=$(cd -- "$(dirname -- "${{BASH_SOURCE[0]}}")/../.." && pwd)
export UV_PYTHON_INSTALL_DIR="$root/.deltaforce/runtime/python" UV_CACHE_DIR="$root/.deltaforce/runtime/uv-cache"
export UV_PYTHON_PREFERENCE=only-managed UV_NO_CONFIG=1 UV_LINK_MODE=copy PYTHONUTF8=1
exec {under_root(paths.uv)} run --quiet --no-project --python {versions["DF_PYTHON_VERSION"]} --with pyyaml --with jsonschema {backslash}
    python {under_root(FRAMEWORK_DIR / "lib" / "py" / "dfcli.py")} --target "$root" "$@"
"""
