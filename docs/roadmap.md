...
- [x] Nothing of the installing machine in what is committed, and the PM speaks like a PM: the guardrail and audit hooks are registered in the committed `.claude/settings.json` with every path through `${CLAUDE_PROJECT_DIR}` (expanded by Claude Code in a hook's command and args), an older install's registration is cleared from `.claude/settings.local.json`, and `.deltaforce/bin/df` resolves the project from its own location - so the registration holds on any machine that installed the project, while what it points at (runtime, framework, policy) stays gitignored and the doctor reports a registration whose runtime is missing, because a hook that cannot start does not block. `.claude/settings.local.json` keeps only what is specific to a machine (Databricks profile, `DF_ROOT`, status line). The PM's tone section now rules out talking about itself - lead with the answer, numbers instead of adjectives, no commentary on its own method, five lines for a status - since the technical depth belongs to the Solution Architect and the builders
...
- [ ] Workspace capabilities probed once and recorded in the conventions, so no role discovers on its own that a rung of the testing ladder is broken ([#33](https://github.com/alessandro9110/deltaforce-ai/issues/33))
...
- [x] ML and GenAI MVP with the Data Scientist and the AI Engineer (M3), run on `deltaforce-ml-sandbox` (issues [#5](https://github.com/alessandro9110/deltaforce-ai/issues/5), [#6](https://github.com/alessandro9110/deltaforce-ai/issues/6), [#7](https://github.com/alessandro9110/deltaforce-ai/issues/7)): F-003 pre-trip fare prediction - linear baseline 1.6480 against LightGBM 1.2730, promotion gate at 85% of the baseline, a later tie rejected and filed as challenger, leakage excluded by construction and re-checked by QA on the published table; F-004 sentiment, `ai_classify` against an LLM with a versioned prompt on 164 reviews neither had seen, macro-F1 0.672 against 0.975 with the whole gap in the lukewarm class; F-005 RAG assistant as a Databricks App with MLflow evaluation, whose G2 became a scope decision (corpus-wide aggregation declared out of the assistant's scope and built exactly in F-009 over the approved sentiment table); F-006 one KPI in a metric view, the AI/BI dashboard and the Genie space as a bundle resource. No Hugging Face model (the PO dropped it at G1) and no Agent Bricks (not on Free Edition)
...
- [ ] **What a delivery team does and ours cannot yet** — gaps found by auditing the roles against a real engagement, one issue each. What a client asks for before looking at anything else: the **cost** of what was built, designed, measured and reported ([#21](https://github.com/alessandro9110/deltaforce-ai/issues/21)), which needs **tags that mean something** instead of the invented ones the M3 run put on its resources ([#22](https://github.com/alessandro9110/deltaforce-ai/issues/22)); **governance and access** — who sees what, sensitive columns, the grants a person must apply — today out of scope in silence ([#23](https://github.com/alessandro9110/deltaforce-ai/issues/23)). What bites in the first week of a real repository: **shared code**, packaged, versioned and reused across jobs ([#24](https://github.com/alessandro9110/deltaforce-ai/issues/24)); **performance and scale**, since everything so far was validated on 21,815 rows ([#25](https://github.com/alessandro9110/deltaforce-ai/issues/25)). What bites at handover: **operational design** — retries, alerts, freshness SLA, a runbook ([#26](https://github.com/alessandro9110/deltaforce-ai/issues/26)); **rollback**, undocumented today ([#27](https://github.com/alessandro9110/deltaforce-ai/issues/27)); **secrets and app authentication** ([#28](https://github.com/alessandro9110/deltaforce-ai/issues/28)); **documentation for the people who use the result** ([#29](https://github.com/alessandro9110/deltaforce-ai/issues/29)). Process: **estimation**, so the PO can plan ([#30](https://github.com/alessandro9110/deltaforce-ai/issues/30)); **cross-feature regression**, because nothing re-runs the tests of delivered features ([#31](https://github.com/alessandro9110/deltaforce-ai/issues/31)); a **risk register** that outlives the design ([#32](https://github.com/alessandro9110/deltaforce-ai/issues/32)); and a **gold modelling standard** ([#34](https://github.com/alessandro9110/deltaforce-ai/issues/34))
