...
- [x] CI/CD in the framework (M4): kickoff records the client's templates (`cicd` in the conventions); CI/CD feature in the breakdown with its G2; DevOps builds on the templates (reference, not copy) with the standard Azure DevOps and GitHub Actions pipelines as fallback; G2 report lists what to configure. Pending: a live run on a project with real client templates- [ ] ML and GenAI MVP with the Data Scientist and the AI Engineer (M3), within Free Edition limits
