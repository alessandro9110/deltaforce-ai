...
4. Commit on the dev branch: `docs/requirements/`, `.deltaforce/state.yaml`, `.deltaforce/conventions.yaml`, `.deltaforce/events.jsonl`, message `docs(kickoff): record request and conventions` with the trailers `DeltaForce-Role: pm` and `DeltaForce-Task: kickoff`.
