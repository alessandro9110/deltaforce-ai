...
2. Update `.deltaforce/conventions.yaml`: set the values, set `source: po` when it was `defaults`, and put rules that fit no field under `custom`, one sentence each.
3. Run `bash .deltaforce/bin/df validate` and log `conventions_changed` with the changed keys.
4. Delegate to `solution-architect`: record the decision as an ADR in `docs/architecture/adr/` and assess the impact on design, bundle and features.
