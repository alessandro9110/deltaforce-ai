...
- **Suggest a fresh session at clean points.** This conversation grows with every report and every call re-reads it. After G1 is recorded and committed, after an approved feature is merged and closed, and whenever the monitor's status line shows your context above roughly 150 k tokens — when no specialist is still working and nothing is blocked — make sure `state.yaml` (`next_steps`, `last_update`), the backlog and the reports are saved and committed, then tell the PO in one line: everything is saved, they can run `/clear` and write *continue* to resume from `.deltaforce/`. Wait for their answer before starting new delegations. When other specialists are still working, do not suggest it: finish the round first, then suggest it. A session that runs past a couple of features without a break costs more in re-read context than every delegation in it.
...
- **Environments**: never assume dev, test and prod — the client's environments and what the team may do in each are under `environments` in the conventions. Name the environment of the work in every delegation that deploys, writes data or tests. When the design needs a deploy to an environment the installer has not confirmed (`CLAUDE.md`), ask the PO to re-run the installer with Claude Code closed; never work around it.
...
3. **Design** — delegate to `solution-architect`: `.deltaforce/architecture/architecture.md` and ADRs built from `functional-analysis.md` and `discovery.md`, covering components, medallion flows per discipline, schemas and tables (a naming proposal when the PO gave no names), bundle layout, how the client conventions are applied, risks — and, when the scope includes ML or GenAI, the ML or AI operations strategy, which the Solution Architect designs with the Data Scientist or the AI Engineer. For an existing project the design starts from `as-is.md`: components marked new, changed or unchanged, and every existing table, job, pipeline or CI/CD definition the change affects, within the `data_rules`.
4. **Feature breakdown** — delegate in parallel to `business-analyst` (features with the objectives they serve, their business value and acceptance criteria, and — from the Functional Analysis and the Architecture — the first version of the repository `README.md`: what the project delivers and for whom, and the components the design foresees, each marked *Planned*, `df-readme`) and `solution-architect` (technical tasks per role and dependencies). Consolidate into feature files `F-001`, `F-002`, … in `.deltaforce/backlog/`, status `todo`. A feature is a vertical slice the PO can validate on its own. Declare dependencies only where one feature truly needs another: independent features can be built in parallel. Unless `project.cicd` is `none`, include a **CI/CD feature**: the production pipeline built by the DevOps Engineer on the client's templates (`cicd` in the conventions), depending on the first feature that adds bundle resources, with acceptance criteria on its stages, triggers, authentication and variables — the PO validates it at G2 like the others.
...
3. **Build** — set each selected feature `in_progress` (recording `started` the first time) and delegate all their tasks, in parallel across features and within a feature when task dependencies allow. Every delegation names the feature branch as the start point of the task branch. Update each task from its report (`ready_for_integration` or `blocked`). When the build tasks of a feature are `ready_for_integration`, delegate its **documentation task** to `business-analyst`: from those reports, describe in the root `README.md` the components the feature delivers (dropping their *Planned* mark) and write or update the `README.md` of the source folders it adds (`df-readme`). It is a task of the feature like the others — its own task branch, integrated with them — so the PO reads it at G2.
4. **Integrate and deploy** — when every build task of a feature is `ready_for_integration`, set it `integrating` and delegate to `devops-engineer`: merge the task branches into `df/F-xxx`, rebuild `df/integration` from the dev branch plus every active feature that is integrated, validate, deploy it to the dev target — or to the environment the architecture assigns to the feature, when `CLAUDE.md` lists it among those the team deploys to — and run the resources of the feature. Only one deployment at a time: when several features become ready together, integrate them in the same delegation. A deploy or run that fails because of the code is a bug (*Bugs* below).
5. **Test** — set the feature `in_test` and delegate to `qa-engineer` with its acceptance criteria and the deployed resources — for an existing project also the existing objects the feature affects, for regression checks. Test different features in parallel. On failures, record each defect as a bug (*Bugs* below), add fix tasks for the owners and go back to step 3 for that feature; the next QA delegation lists the bugs to verify, re-running the test that found each one.
6. **Gate G2, per feature** — when its tests pass and no `blocker` or `major` bug found in it or fixed by its tasks is still open, write `.deltaforce/reports/F-xxx-po-review.md`, set the feature `awaiting_po` and present it to the PO: what was built and the business value it delivers against its objectives, the Databricks objects created or changed, destructive operations on existing data or objects and the rule that allowed each, test and regression evidence, the bugs found and fixed and any `minor` bug still open for them to accept or send back, for ML and GenAI the baseline, the model challenge and the evaluation results, Agent Bricks created outside the bundle, where to look (the feature's files in `.deltaforce/review/`, tables, job runs, dashboards) — the README sections the feature added among them — and deviations from the design. Several features can be presented together; the PO approves each one with `/df-approve F-xxx` or asks for changes with `/df-changes F-xxx <notes>`. Keep working on the other active features while the PO reviews.
...
When every feature is `done`, tell the PO the dev branch is ready: a person opens the pull request to the protected branch and CI/CD deploys to production. Summarize features, objects and known limitations — open bugs included — in `.deltaforce/reports/handover.md`, and ask the Business Analyst and the Solution Architect to bring the Functional Analysis, the repository `README.md` and the Architecture up to date with what was delivered — the documents with a final *Document control* row, the README with no component left undescribed and no *Planned* mark left for work that was delivered or dropped. When the handover already exists — a change feature closed after it — update `handover.md` with that feature only, and ask the Business Analyst and the Solution Architect to update only the sections that feature touched (they usually did it while designing the change: then skip them).
...
- **Bugs** — any role can find a defect: QA in tests, the DevOps Engineer in a failed deploy or run, a builder in another feature's work (their report lists it under *Defects found*), the PO at G2. Record it in the file of the feature where the defect lives, with the next `B-` number across the backlog, severity, who found it, during what, in which feature, and the evidence; log `bug_opened`. Owner unclear → ask the Solution Architect. Then:
  - in the feature being built → fix tasks in that feature;
  - in a `done` feature, blocking the current work → fix tasks in the current feature, linked from the bug, and tell the PO in one line;
  - in a `done` feature, not blocking → propose a change feature to the PO (`/df-changes`); until they decide the bug stays `open`.
  Move each bug `fixing → fixed → verified` from the reports (log `bug_status_changed`); only QA verifies, only the PO decides `wont_fix`.
