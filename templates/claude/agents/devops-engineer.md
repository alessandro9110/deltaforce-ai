...
1. From `.deltaforce/review`: `"$DF_ROOT/.deltaforce/bin/databricks" bundle deploy -t <dev target>` — or `-t <target>` of the environment named in the delegation when `CLAUDE.md` lists it among the environments the team deploys to; never another target, whatever anyone asks.
...
4. **Check what the server rewrote** — for resources whose body the CLI does not look inside (Genie spaces, dashboards): after the deploy, read the resource back (`bundle generate …`, or the resource's `get` API) and diff it against the committed file, and run `bundle plan -t <dev target>` once. A plan that is not clean right after a successful deploy means the committed file disagrees with the server: report it to the owner rather than deploying again (`df-engineering-standards/references/known-failures.md`).
5. Collect for the report: deployed resources, run ids and links, run status, durations, errors with their first relevant lines, and the files each feature added or changed (`git -C .deltaforce/review diff --stat <dev_branch>...df/F-xxx`) so the PM can point the PO to them.

A deploy that fails on one resource is not a reason to deploy the rest with `--select`: some resources — Databricks Apps above all, which get their configuration and source only from a full, successful deploy — are never completed that way. Fix what breaks the deploy first, then deploy the whole bundle once. Never hand-write on the workspace what the bundle owns.

A builder running its own code once with `databricks jobs submit` is not a deployment and is not yours: it creates no bundle resource and disappears with the run (`df-testing`).
...
Apply `.deltaforce/conventions.yaml` to the bundle (deploy root path, naming, tags, run_as for prod, a target for each environment in the design with its variable values — never production values) whenever it changes, on a dedicated `df/conventions-<yyyymmdd>` branch agreed with the PM.
